...
const DEMO_PHOTOS = [
  { id: 'demo-1', people_count: 1, price: 10, preview_url: 'https://images.unsplash.com/photo-1511632765486-a01980e01a18?w=1200&q=80' },
  { id: 'demo-2', people_count: 4, price: 30, preview_url: 'https://images.unsplash.com/photo-1517457373958-b7bdd4587205?w=1200&q=80' },
  { id: 'demo-3', people_count: 8, price: 30, preview_url: 'https://images.unsplash.com/photo-1519167758481-83f550bb49b3?w=1200&q=80' },
  { id: 'demo-4', people_count: 2, price: 30, preview_url: 'https://images.unsplash.com/photo-1519741497674-611481863552?w=1200&q=80' },
  { id: 'demo-5', people_count: 1, price: 10, preview_url: 'https://images.unsplash.com/photo-1531058020387-3be344556be6?w=1200&q=80' },
  { id: 'demo-6', people_count: 6, price: 30, preview_url: 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=1200&q=80' },
];
...
export default function Home() {
    ...
    try {
      ...
      for (const photo of photos) {
        try {
          ...
          if (best < 0.53) results.push({ ...photo, match_distance: best, people_count: Math.max(Number(photo.people_count || 1), faces.length), price: Math.max(Number(photo.people_count || 1), faces.length) > 1 ? 30 : 10 });
        } catch (err) {
  ...
  return <main className="shell">
    ...
    {mode === 'customer' ? <>
      ...
      {!matched ? <section className="howItWorks"><div className="howIntro"><span className="eyebrow">HOW IT WORKS</span><h2>One selfie. Your gallery.</h2></div><div className="howGrid"><Feature icon={<Search/>} n="01" title="Match your face" copy="Your selfie is compared with faces detected in event photos."/><Feature icon={<Lock/>} n="02" title="Preview securely" copy="Customers only receive a downscaled preview, never the original."/><Feature icon={<IndianRupee/>} n="03" title="Buy what you want" copy="₹10 single photo · ₹30 group photo · originals unlock after approval."/></div></section> : <section className="resultsSection"><div className="resultHeader"><div><span className="eyebrow">MATCH RESULTS</span><h2>Your photos <span>· {displayPhotos.length} matches</span></h2></div><div className="resultTrust"><Lock size={14}/> Originals locked</div></div><div className="photoGrid">{displayPhotos.map((p) => <button key={p.id} className={`photoCard ${selected.includes(p.id) ? 'chosen' : ''}`} onClick={() => toggle(p.id)}><img src={p.preview_url} alt="Event preview" crossOrigin="anonymous"/><div className="blurLayer"/><div className="watermark">SMF STUDIO</div><div className="photoGradient"/><div className="photoBottom"><span>{Number(p.people_count || 1) > 1 ? 'Group' : 'Single'} · {p.people_count || 1} {(p.people_count || 1) === 1 ? 'face' : 'faces'}</span><b>₹{p.price || 10}</b></div>{selected.includes(p.id) && <div className="selectedBadge"><Check size={16}/></div>}<div className="previewLock"><Lock size={13}/> PREVIEW</div></button>)}</div><div className="pricingRow"><div><b>Single photo</b><span>₹10</span></div><div><b>Group photo</b><span>₹30</span></div><div className="pricingNote"><ShieldCheck size={18}/> Full-resolution originals remain private.</div></div></section>}
